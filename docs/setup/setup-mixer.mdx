...
* If the Blade angle is different from the Override angle, increase or decrease [**Collective Calibration %**](../configurator/tabs/mixer.mdx#cyclic-and-collective-calibration) until the bade angle matches.
...
* Increase or decrease the [**Cyclic Calibration %**](../configurator/tabs/mixer.mdx#cyclic-and-collective-calibration)
...
* Set mixer yaw override to 22deg CW/CCW, adjust [**Yaw Calibration %**](../configurator/tabs/mixer.mdx#tail-rotor-settings) until you have around 45deg both ways.\
  **Note** 22 is close enough to 1/2 of 45 deg.
* Set mixer yaw override to 60deg CW/CCW, adjust [**CW / CCW blade angle limits**](../configurator/tabs/mixer.mdx#tail-rotor-settings) to the maximum number that does not causing binding.
