...
<video src="./img/arming-2.mp4" autoplay playsinline loop muted width="85%"></video>
...
<video src="./img/arming-3.mp4" autoplay playsinline loop muted width="85%"></video>

## Swashplate Wiggle indication

The swashplate is configured to indicated ready for Arming as well as Errors. More details can be found on the [Swashplate wiggle](../setup/swash-wiggle.md) page.
...
|   'THROTTLE'        | Lower throttle below [Throttle Channel value for arming](../configurator/tabs/receiver.md#receiver-settings)                 |
...
|   'RPMFILTER'       | RPM-based filtering is enabled but one or more ESC's are not configured or receiving valid RPM signals. RPM feedback can be from bi-directional Dshot or Frequency inputs using the ESC RPM signal or an external RPM sensor. See [RPM Measurement](../setup/rpm-measurement.md) for more details.|
