...
!!! note "Windows Drivers"
    Windows needs the right drivers for flashing Rotorflight. If you haven't flashed Rotorflight before or if the above procedure fails, refer to the instructions under [**Flashing the Firmware**](../../setup/flashing-the-firmware.md).


## 2. Configure the F4-mini for the S2

The Rotorflight team has created a tune for the S2 Max and Ultra. It's available as a diff and as a dump.
RPMs for profile 1/2/3: 4400/5100/5500.

* S2 [configuration diff](../configs/f4mini-s2-diff.txt)
* S2 [configuration dump](../configs/f4mini-s2-dump.txt)
...
7. Go to the *Servos* tab and [adjust servo centers](../../setup/setup-servos.md#step-3---set-arms-at-90-degrees).
8. Go to the *Mixer* tab and [check the *Collective trim*](../../setup/setup-mixer.md#level-the-swashplate-and-blade-pitch-to-zero)
9. On the *Rates* tab, configure the [rates](../../configurator/tabs/rates.md) to your liking.
10. On the *Modes* tab, configure the [modes](../../configurator/tabs/modes.md) so that they work with your transmitter.
11. On the *Adjustments* tab, configure the [adjustments](../../configurator/tabs/adjustments.md) so that they work with your transmitter.
...
1. Download [`s2-main-esc-config.bin`](../configs/s2-main-esc-config.bin)
...
1. Download [`s2-tail-esc-config.bin`](../configs/s2-tail-esc-config.bin)
2. Click on *Apply config*
