...
:::note
This is the official Rotorflight tune for the Goosky S2 Ultra. The S2 Ultra and S2 Max use different tunes—select the correct one for your model. This tune requires updated AM32 ESC firmware, so update your ESC before applying.
:::
...
The Rotorflight team has created a tune for the S2 Ultra. It's available as a diff and as a dump.
RPMs for profile 1/2/3: 4400/5100/5500.

* S2 Ultra [configuration diff](../configs/f4mini-s2-ultra-diff.txt)
* S2 Ultra [configuration dump](../configs/f4mini-s2-ultra-dump.txt)
...
1. Download [`s2-ultra-main-esc-config.bin`](../configs/s2-ultra-main-esc-config.bin)
2. Click on *Apply config*
3. Click *Choose file* and select the file `S2-ultra-main-esc-config.bin` that you just downloaded
4. Only select ESC 1
5. Click *Apply*

Now configure the tail ESC.

1. Download [`s2-ultra-tail-esc-config.bin`](../configs/s2-ultra-tail-esc-config.bin)
2. Click on *Apply config*
3. Click *Choose file* and select the file `S2-ultra-tail-esc-config.bin`
4. Only select ESC 2
5. Click *Apply*
6. Click *Disconnect*
7. Power down the S2
