# Configuration

## Personalization[​](#personalization "Direct link to Personalization")

Allows you to set your heli name to be shown in the OSD, blackbox logs and diff/dump outputs.

![Configuration Tab](data:image/png;base64,)

## System Configuration[​](#system-configuration "Direct link to System Configuration")

![Configuration Tab](/rotorflight-docs/assets/images/config-sys-conf-1cd8f72e7026b90bd4d02c3ce70f1c36.png)

### Gyro Update Frequency[​](#gyro-update-frequency "Direct link to Gyro Update Frequency")

This indicates the current gyro update frequency.

### PID Loop Frequency[​](#pid-loop-frequency "Direct link to PID Loop Frequency")

This is the frequency that the control loop operates at. Set to between 1kHz and 2kHz.

There is limited or no benefit in higher rates and will result in high loads for your flight controller. This could be an issue for F405 and F411 boards.

### Accelerometer[​](#accelerometer "Direct link to Accelerometer")

This must be selected if you wish to use stability (6G modes) such as Angle, Horizon or Rescue modes.

### Barometer[​](#barometer "Direct link to Barometer")

This is used for Altitude hold Rescue which is currently still being developed

### Magnetometer[​](#magnetometer "Direct link to Magnetometer")

Currently this does not provide any control functionality, however, it can be used for telemetry.

## Features[​](#features "Direct link to Features")

This section is used to enable or disable features.

![Configuration Tab](/rotorflight-docs/assets/images/config-features-b815e995ab63782eb70b8cc4d1e24b21.png)

tip

Features such as the GPS must be selected in the [Serial ports](#serial-ports) section before selecting the Feature here.

## Serial Ports[​](#serial-ports "Direct link to Serial Ports")

Please choose the functions you wish to assign to your UART.

![Configuration Tab](/rotorflight-docs/assets/images/config-serial-2e4ff7916b56a95c6be0d5db60c23569.png)

### MSP[​](#msp "Direct link to MSP")

As the name suggests, usually only used for lower-level communication using the MSP (MultiWii Serial Protocol) for configuration done by external devices, or to allow a more direct way of control. You can also set a specific baud rate.

### GPS[​](#gps "Direct link to GPS")

GPS is not currently used for Rotorflight for any flight controls. This can be used for telemetry purposes.

### Serial Rx[​](#serial-rx "Direct link to Serial Rx")

Check [Receiver](/rotorflight-docs/docs/configurator/tabs/receiver.md) page for detailed info.

### ESC Telemetry[​](#esc-telemetry "Direct link to ESC Telemetry")

Check [ESC Telemetry](/rotorflight-docs/docs/setup/esc-telemetry.md) page for detailed info.

### Blackbox Logging[​](#blackbox-logging "Direct link to Blackbox Logging")

This enables a UART for an external logging device such as [OpenLager](/rotorflight-docs/docs/setup/openlager.md).

### Telemetry: FrSky SmartPort[​](#telemetry-frsky-smartport "Direct link to Telemetry: FrSky SmartPort")

This is used when connecting an FrSky SmartPort receiver pin to this UART.

## Board and Sensor alignment[​](#board-and-sensor-alignment "Direct link to Board and Sensor alignment")

Change the roll, pitch and yaw gyro alignment value until the model on the status page moves in the same direction as the gyro board.

![Configuration Tab](/rotorflight-docs/assets/images/config-sens-align-b42fdf342e76f9b151e7a7236d216d49.png)

## Accelerometer Trim[​](#accelerometer-trim "Direct link to Accelerometer Trim")

This section is used to trim the roll and pitch axis while the helicopter is in Stability mode (e.g Angle, horizon or Rescue).

The Transmitter trims for each cyclic axis must be at neutral when using Rotorflight (at 1500us). This will usually result in the helicopter drifting while in a stable hover. To correct for this drift the Accelerometer roll and pitch trims are to be used.

This can be done either directly by the configurator or via the transmitter with [Lua](/rotorflight-docs/docs/setup/using-stability-modes-example.md) or Adjustments.

![Configuration Tab](data:image/png;base64,)
