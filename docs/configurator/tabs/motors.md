...
!!! note
    ...
    The FC also needs to know the [RPM](../../setup/rpm-measurement.md) of the main rotor, and the tail rotor. For this you have to exactly specify the *Motor pole count* and the *Gear ratios*.
...
This switch is only visible when the *ESC/Motor protocol* is DSHOTxxx. It enables motor eRPM telemetry on supported ESCs via [bidirectional DSHOT](../../setup/rpm-measurement.md#bi-directional-dshot) and can be used to facilitate the *GOVERNOR* and/or *RPM\_FILTER* features. If you use bidirectional DSHOT you don't have to use a frequency sensor for obtaining the eRPM of the motor(s).
