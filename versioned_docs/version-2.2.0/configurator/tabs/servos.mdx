...
For 120, 135 and 140 degree CCPM swashplates.
...
The bobbing happens because the elevator servo has to move twice the distance vs. the aileron servos, when elevator position is changed.
...
To tune this value increase the speed value (the units are ms/60°, as in the servo datasheet) until the bobbing is not visible anymore. It should be always OK to use the value from the servo datasheet if unsure.
